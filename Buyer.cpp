...
void Buyer::showOrder()
{
    ...
    cout<<"以下為您的訂單:"<<endl;
    cout << "ID\tProduct\tSeller\tAmount\tPayment"<<endl;
...
void Buyer::confirmOrder()
{
    ...
    while(true)
    {
        ...
        if(choose == 1)
        {
            ...
            Statement* stmt = conn->createStatement();
            string query = "update orders set payment = '" + payment + "' where id =" + to_string(id); 
            stmt->executeUpdate(query);
        }
        else
        {
